...
namespace UnitTests
{
    ...
    public class StreamReaderWrapperTests
    {
        ...
        {
            IReader reader = new MemoryStream(new byte[] { 1, 2, 3, 4 }).Reader();
            var buf = new byte[4];
        ...
        {
            IReader reader = new MemoryStream(new byte[] { }).Reader();
            var buf = new byte[4];
            var res = reader.Read(buf);
        ...
        {
            IReader reader = new MemoryStream(new byte[] { 1, 2 }).Reader();
            var buf = new byte[2];
            var res = reader.Read(buf);
